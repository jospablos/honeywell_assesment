...
export default function DefaultInput({ id, label, error, type = 'text', value, onChange: onChangeProp, validate, enableNextStep, description, autoFocus = true }) {
    ...
    return (
        <div className={classnames("DefaultInput-container", { error: hasFocusedOut && error })}>
            <label>{label}</label>
            <p>{description}</p>
            <input autoFocus={autoFocus} value={value} type={type} onChange={onChange} onBlur={onBlur} />
